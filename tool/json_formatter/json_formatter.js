...
const format_structure = {
	indent_type: {
		...
		option: {
			...
			other: {
				display_name: 'その他の文字列',
				children: {
					indent_text: {
						display_name: 'インデント文字列',
						max: 10,
						type: 'string',
						placeholder: '0~10文字',
					},
...
const gui_structure = {
	type: {
		type: 'select',
		display_name: 'トップレベルの構造',
		option: {
			object: {
				display_name: 'オブジェクト',
				children: { object: { type: 'reference', name: 'object_children' } },
			},
			array: {
				display_name: 'リスト',
				children: { array: { type: 'reference', name: 'array_children' } },
			},
		},
	},
}
const select_type_structure = {
	type: {
		...
		option: {
			string: {
				children: {
					default: { type: 'toggle', display_name: 'デフォルトの値', children: { type: 'string' } },
				},
			},
			number: {
				children: {
					default: { type: 'toggle', display_name: 'デフォルトの値', children: { type: 'number' } },
				},
			},
			bool: {
				children: {
...
document
	.getElementById('generate-format-text-button')
	.addEventListener('click', generate_format_from_input_json)
// execute
...
function change_format_gui() {
	document.getElementById('input-format-gui-area').textContent = JSON.stringify(
		structure.get('input-format-gui-structure').gui,
		null,
		2
	)
}
...
function execute() {
	...
	if (structure_type === 'no') {
		...
		log('フォーマット : TEXT')
		const format_text_data = parse_json(
			document.getElementById('input-format-area').value,
			'入力されたフォーマット'
		)
	...
	if (['string', 'number', 'boolean', 'null'].includes(type)) {
		if (target === undefined) {
			if (format.default === undefined) {
				...
				return format_json(format.default, format)
			}
		}
		try {
			if (type === 'string') {
		...
		if (type === 'object') {
			...
			if (typeof target !== 'object' || target === null || Array.isArray(target)) {
				throw new Error(`type:object フォーマット対象がオブジェクトではありません`)
			}
			const sort = format.sort
			const entry = format.entry
			if (entry === undefined) {
				throw new Error(`type:object entryの値が見つかりません`)
			}
			if (!Array.isArray(entry)) {
				throw new Error(`type:object entryが配列ではありません`)
			}
			if (sort === true) {
				entry.sort((a, b) => {
			...
			entry.forEach((entry_format, i) => {
				...
				if (key === undefined) {
					throw new Error(`type:object entryの${i}番目 keyが見つかりません`)
				}
			...
			if (!Array.isArray(target)) {
				throw new Error(`type:array フォーマット対象が配列ではありません`)
			}
			let result = []
