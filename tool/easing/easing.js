...
document.getElementById('code-copy').addEventListener('click', function () {
	...
	this.innerHTML =
		'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M4,13 10,19 21,7" /></svg>'
	setTimeout(() => {
		this.classList.remove('clicked')
		this.innerHTML =
			'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" fill="none" stroke="currentColor"><path d="M8,8v-2a3,3 0 0 1 3-3h7a3,3 0 0 1 3,3v7a3,3 0 0 1-3,3h-2v2a3,3 0 0 1-3,3h-7a3,3 0 0 1-3-3v-7a3,3 0 0 1 3-3h7a3,3 0 0 1 3,3v5" /></svg>'
	}, 1000)
})
...
function update_graph() {
	...
	ctx.bezierCurveTo(
		points[0].x * graph_draw_size,
		(1 - points[0].y) * graph_draw_size,
		points[1].x * graph_draw_size,
		(1 - points[1].y) * graph_draw_size,
		graph_draw_size,
		0
	)
	ctx.stroke()
...
function update_code() {
	document.getElementById('code-content').textContent =
		`cubic-bezier(${points[0].x}, ${points[0].y}, ${points[1].x}, ${points[1].y})`
}
