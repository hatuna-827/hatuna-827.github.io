...
document.getElementById('export-execute-button').addEventListener('click', function () {
	...
	document.querySelectorAll('#export-select-box .export-checkbox').forEach(checkbox => {
		...
		if (remove) {
			wordbook_data.splice(checkbox.dataset.index - count, 1)
		}
		count++
	})
	save_wordbook_data()
...
function edit_words(index) {
	...
	wordbook_data[index].words.forEach((word_content, i) => {
		...
		word_add.innerHTML =
			'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" fill="none" stroke-width="1.5"><path d="M3,12h18M12,3v18" /></svg>'
...
function dots_click(e) {
	...
	title_edit.addEventListener('click', () => {
		...
		if (new_title_name != null) {
			wordbook_data[document.getElementById('dots-menu').parentElement.dataset.index].name =
				new_title_name
			save_wordbook_data()
		}
		display_title()
	...
	title_remove.addEventListener('click', () => {
		dialog({
			...
			if (resolve == 1) {
				obj_manip.remove(wordbook_data, [
					document.getElementById('dots-menu').parentElement.dataset.index,
				])
				save_wordbook_data()
				display_title()
			}
