	...
	setting: {
		...
		children: {
			...
			height: {
				type: 'toggle',
				display_name: '高さ',
				children: { type: 'number', default: 24, min: 0 },
			},
			width: {
				type: 'toggle',
				display_name: '幅',
			...
			x: {
				type: 'toggle',
				display_name: 'x軸の相対位置',
				children: { type: 'number', default: 0, min: 0 },
			},
			y: {
				type: 'toggle',
				display_name: 'y軸の相対位置',
				children: { type: 'number', default: 0, min: 0 },
			},
			...
			children: {
				...
				shape: {
					...
					option: {
											...
											type: {
												...
												option: {
													M: {
														display_name: '移動(絶対)',
														children: {
															reference: { type: 'reference', name: 'One_point_children' },
														},
													},
													L: {
														display_name: '直線(絶対)',
													...
													H: {
														display_name: '水平(絶対)',
														children: { reference: { type: 'reference', name: 'x_children' } },
													},
													V: {
														display_name: '垂直(絶対)',
														children: { reference: { type: 'reference', name: 'y_children' } },
													},
													A: {
														display_name: '弧(絶対)',
														children: { reference: { type: 'reference', name: 'A_children' } },
													},
													m: {
														display_name: '移動(相対)',
														children: {
													...
													Q: {
														display_name: '2次ベジエ曲線(絶対)',
														children: {
															reference: { type: 'reference', name: 'Two_point_children' },
														},
													},
													q: {
														display_name: '2次ベジエ曲線(相対)',
														children: {
															reference: { type: 'reference', name: 'Two_point_children' },
														},
													},
													C: {
														display_name: '3次ベジエ曲線(絶対)',
														children: {
															reference: { type: 'reference', name: 'Three_point_children' },
														},
													},
													c: {
														display_name: '3次ベジエ曲線(相対)',
														children: {
															reference: { type: 'reference', name: 'Three_point_children' },
														},
													},
						...
						polyline: {
							display_name: '折れ線 (polyline)',
							children: { points: { type: 'reference', name: 'points' } },
						},
						polygon: {
							display_name: '多角形 (polygon)',
							children: { points: { type: 'reference', name: 'points' } },
						},
						text: {
							display_name: 'テキスト (text)',
							children: {
								text: { type: 'string' },
		...
		children: {
			...
			children: {
				fill: {
					type: 'toggle',
					display_name: '塗りつぶし',
					children: { type: 'string', placeholder: 'black , #000' },
				},
				'fill-opacity': {
					type: 'toggle',
					display_name: '塗りつぶし透明度',
					children: { type: 'number', min: 0, max: 1, step: 'any', placeholder: '0~1' },
				},
				stroke: {
					type: 'toggle',
					display_name: '線の色',
				...
				'stroke-width': {
					type: 'toggle',
					display_name: '線の幅',
					children: { type: 'number', min: 0, step: 'any' },
				},
				'stroke-dasharray': {
					type: 'toggle',
					display_name: '破線パターン',
					children: {
						type: 'string',
						pattern: '[0-9]+,[0-9]+',
						placeholder: '破線の長さ,空白の長さ',
					},
				},
				'stroke-linecap': {
					type: 'toggle',
					display_name: '線の終端の形状',
					children: {
						type: 'select',
						option: {
							butt: { display_name: '直角(butt)' },
							round: { display_name: '半円(round)' },
							square: { display_name: '伸ばして直角(square)' },
						},
					},
				},
				'stroke-linejoin': {
					type: 'toggle',
					display_name: '2つの線のつなぎめの形状',
					children: {
						type: 'select',
						option: {
							miter: { display_name: '直角(miter)' },
							round: { display_name: '丸(round)' },
							bevel: { display_name: '斜角(bevel)' },
						},
					},
				},
			},
...
const Two_point_children = {
	control: {
		type: 'object',
		display_name: '制御点',
		children: { point: { type: 'reference', name: 'One_point_children' } },
	},
	stop: {
		type: 'object',
		display_name: '終点',
...
const A_children = {
	...
	'large-arc-flag': {
		type: 'select',
		display_name: '大小',
		option: { 0: { display_name: '小さい方' }, 1: { display_name: '大きい方' } },
	},
	'sweep-flag': {
		type: 'select',
		display_name: '描く向き',
		option: { 0: { display_name: '反時計回り' }, 1: { display_name: '時計回り' } },
	},
...
document.getElementById('main-xml-textarea').addEventListener('input', function () {
	...
	if (xml_editor.value == '') {
		xml_editor.value = default_xml_value
	}
...
document.getElementById('svg-download').addEventListener('click', function () {
	...
	download.setAttribute(
		'href',
		URL.createObjectURL(
			new Blob([comp_xml(document.getElementById('xml-sub-view').textContent)], {
				type: 'text/plain',
			})
		)
	)
...
function open_setting() {
	...
	opening_window = window.open(
		'./settings.html',
		'_blank',
		'top=100,left=200,height=500,width=400,popup'
	)
...
function update_line_numbers(value, target) {
	...
	if (line_count) {
		line_count = line_count.length + 1
	} else {
		line_count = 1
	}
	for (let i = 0; i < line_count; i++) {
		target.insertAdjacentElement('beforeend', document.createElement('span'))
	}
}
function object_to_svg(obj) {
	const shape_options = svg_struct.element.children.children.shape.option
	const d_option =
		svg_struct.element.children.children.shape.option.path.children.d.children.children.type.option
	...
	elements.forEach(element => {
		...
		if (element.shape === 'path') {
			...
			element.d.forEach((d, i) => {
				...
				xml += type
				if (d_option[type].children === undefined) {
					return
				}
