...
function def_struct(name, struct) {
	if (name === undefined) {
		error(`|Structure module| Not enough arguments (def_struct > name)`)
	}
	if (struct === undefined) {
		error(`|Structure module| Not enough arguments (def_struct > struct name:${name})`)
	}
	structs[name] = struct
}
function set(id, struct_name, change_func) {
	if (id === undefined) {
		error(`|Structure module| Not enough arguments (set > id)`)
	}
	if (struct_name === undefined) {
		error(`|Structure module| Not enough arguments (set > struct_name id:${id})`)
	}
	const pos = document.getElementById(id)
	if (pos === null) {
		error(`|Structure module| No element found with this id (${id})`)
	}
	const struct = structs[struct_name]
	if (struct === undefined) {
		error(`|Structure module| No struct found with this struct name (${struct_name})`)
	}
	...
	Object.keys(struct).forEach(name => {
		...
		if (['string', 'number'].includes(type)) {
			...
			if (value.name === undefined) {
				error(`|Structure module| No name found in reference`)
			}
			if (structs[value.name] === undefined) {
				error(`|Structure module| No struct found with this name (${value.name})`)
			}
			const field = generate_field('div', type)
...
function add_input(pos, value, path, change_func) {
	...
	if (['string', 'number'].includes(type)) {
		...
		if (children === undefined) {
			error(`|Structure module| No children found in toggle`, value)
		}
		const default_value = value.default
		...
		const children = value.children
		if (children === undefined) {
			error(`|Structure module| No children found in list`, value)
		}
		const field_UUID = crypto.randomUUID()
		const min = value.min
		const max = value.max
		...
		btn_data.forEach(data => {
			...
			data.element.addEventListener('click', function () {
				...
				remove_btn.addEventListener('click', function () {
					if (nest.childElementCount <= min) {
						return
					}
					const target_UUID = obj_manip
						.get(structs_UUID, field_UUID)
						.splice(Array.from(nest.children).indexOf(remove_field), 1)
				...
				if (data.class === 'begin') {
					obj_manip.get(structs_UUID, field_UUID).splice(0, 0, {})
				}
				add_input(
					remove_field,
					children,
					[field_UUID, data.class === 'begin' ? 0 : nest.childElementCount],
					change_func
				)
		...
		for (let i = 0; i < min; i++) {
			btn_data[1].element.click()
		}
	} else if (type === 'object') {
		const children = value.children
		if (children === undefined) {
			error(`|Structure module| No children found in object`, value)
		}
		const nest = document.createElement('div')
		...
		if (options === undefined) {
			error(`|Structure module| No option found in select`, value)
		}
		if (Array.isArray(options)) {
			options = Object.fromEntries(options.map(key => [key, {}]))
		}
		...
		if (option_keys.length === 1) {
			...
			if (children === undefined) {
				error(`|Structure module| No children found in option`, value)
			}
			const default_value = value.default
			const input = document.createElement('input')
			input.id = field_UUID
			...
			btn.addEventListener('click', function () {
				...
				if (input.checked === true) {
					add_field(field, children, [...path, 'children'], change_func)
				}
				change_func()
			})
			if (default_value === true) {
			...
			input.addEventListener('change', function () {
				obj_manip.set(structs_UUID, [...path, 'children'], {})
				field.innerHTML = ''
				if (options[this.value].children !== undefined) {
					add_field(field, options[this.value].children, [...path, 'children'], change_func)
				}
				change_func()
			})
			if (options[input.value].children !== undefined) {
				add_field(field, options[input.value].children, [...path, 'children'], change_func)
			}
			pos.appendChild(input)
		}
		...
		if (name === undefined) {
			error(`|Structure module| No name found in reference`, value)
		}
		if (structs[name] === undefined) {
			error(`|Structure module| No struct found with this name (${name})`)
		}
		const field = document.createElement('div')
...
function get(id) {
	if (id === undefined) {
		error(`|Structure module| Not enough arguments (get > id)`)
	}
	const structs = JSON.parse(JSON.stringify(structs_UUID))
	const list_keys = Object.keys(structs)
	if (!list_keys.includes(id)) {
		error(`|Structure module| No id found (get > ${id})`)
	}
