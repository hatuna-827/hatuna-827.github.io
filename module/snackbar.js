...
export function snackbar({
	...
	if (!color || !bg_color || !icon) {
		...
		if (!['SUC', 'WARN', 'ERR', 'INFO'].includes(type)) {
			type = 'SUC'
		}
		if (!context) {
			context = type
		...
		icon =
			'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke="white"><circle cx="12" cy="12" r="12" fill="currentColor" stroke-width="2" stroke="none" /><path d="' +
			design.icon +
			'" /></svg>'
	}
	if (!context) {
