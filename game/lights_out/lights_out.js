...
function push(x, y) {
	...
	out()
	if (
		JSON.stringify(log) == JSON.stringify(random_point) ||
		(JSON.stringify(log) == JSON.stringify(save_point) &&
			JSON.stringify(data) == JSON.stringify(nul))
	) {
...
function answer() {
	...
	for (let x = 0; x < 3; x++) {
		for (let y = 0; y < 3; y++) {
			if (point[x][y] == 1) {
				document.getElementById(x * 3 + y).textContent = '・'
			} else {
				document.getElementById(x * 3 + y).textContent = ''
			}
		}
