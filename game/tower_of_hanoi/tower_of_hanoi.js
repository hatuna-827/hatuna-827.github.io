...
function reflect_setting() {
	...
	if (setting.display_index) document.getElementById('boxs').classList.add('display-index')
	else document.getElementById('boxs').classList.remove('display-index')
}
		...
		if (select_index === index) {
			...
			if (index !== 0 && disk_data[index].length === step_count && start_time !== null) {
				...
				document.getElementById('time').textContent =
					`${Math.floor(time / 1000)}.${('00' + (time % 1000)).slice(-3)}`
				document.getElementById('clear').textContent =
					pole_count === 3 && move_count === 2 ** step_count - 1
						? 'ゲームクリア 最短！'
						: 'ゲームクリア'
			}
			render_disks()
